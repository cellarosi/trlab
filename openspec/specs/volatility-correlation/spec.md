...
Provide a repository script that measures whether a ticker's daily range (a
volatility proxy) on day `t` is correlated with its range on a later day `t+lag`,
using credential-free historical daily bars from the `feed` package.
